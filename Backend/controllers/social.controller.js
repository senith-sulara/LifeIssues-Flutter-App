  ...
  '/insert',
    async (req, res) => {
      ...
      res.send('Issue details uploaded successfully.');
        ...
        res.status(400).send('Error while uploading Socials. Try again later.');
      }
    },
    (error, req, res, next) => {
      if (error) {
...
Router.get('/getAllIssues', async (req, res) => {
  try {
    const files = await Socials.find({});
    const sortedByCreationDate = files.sort(
      (a, b) => b.createdAt - a.createdAt
    );
    res.send(sortedByCreationDate);
  } catch (error) {
    res.status(400).send('Error while getting list of Socials. Try again later.');
  }
});
...
Router.get('/getIssuesaby', async (req, res) => {
  try {
    ...
    const files = await Socials.find({age: {$gt:17, $lt:24}});
    const sortedByCreationDate = files.sort(
      (a, b) => b.createdAt - a.createdAt
    );
    res.send(sortedByCreationDate);
    ...
    const files = await Socials.find({age: {$gt:23, $lt:30}, gender:'Male', civilstatus: 'Unmarried'});
    const sortedByCreationDate = files.sort(
      (a, b) => b.createdAt - a.createdAt
    );
    res.send(sortedByCreationDate);
  }else if(userData.age >= 24 && userData.age < 30 && userData.gender == 'Male'  && userData.civilstatus == 'Married'){
    const files = await Socials.find({age: {$gt:23, $lt:30}, gender:'Male', civilstatus: 'Married'});
    const sortedByCreationDate = files.sort(
      (a, b) => b.createdAt - a.createdAt
    );
    res.send(sortedByCreationDate);
    ...
    const files = await Socials.find({age: {$gt:23, $lt:30}, gender:'Female', civilstatus: 'Unmarried'});
    const sortedByCreationDate = files.sort(
      (a, b) => b.createdAt - a.createdAt
    );
    res.send(sortedByCreationDate);
...
Router.put("/:id", async (req, res) => {
  try {
    let issue = await Socials.findById(req.params.id);
    ...
    issue = await Socials.findByIdAndUpdate(req.params.id, data, { new: true });
    res.json(issue);
  } catch (e) {
...
Router.delete("/:id", async (req, res) => {
  try {
    // Find issue by id
    const issue = await Socials.findById(req.params.id);
